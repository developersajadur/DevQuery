...
//  const [answers, setAnswers] = useState([])
  ...
  const handleCommentSubmit = async (e, answerId) => {
    ...
    const comment = formData.get('comment'); // Get the comment from the form
 ...
 if(isError){
   return console.log("answern rendering err", isError)
 }
 if(isLoading){
  return (
    <Loading></Loading>
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        {answers.length > 0 ? (
                ...
                <div className="ml-4 w-full">
                  ...
                  <div className="mt-4">
                    <form onSubmit={(e) => handleCommentSubmit(e, answer._id)}>
                      <Textarea
                        name="comment" // No need for state or onChange
                      ...
                      <div className='flex items-center justify-between px-2 my-3'>
                        ...
                        <Link url = {url}
                          ...
                          href={{ pathname: `/allcomments/${answer._id}`, query: { referrer: url } }}
                        >
                          See all comments
                        </Link>
                      </div>
          ...
          <p className="text-gray-500 font-semibold text-center">No answers yet.</p>
        )}
      </div>
    </div>
    </div>
