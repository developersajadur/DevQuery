...
export async function POST(req) {
  const { name, email, platform, meetingLink, description, date, time } = await req.json();

  if (!name || !email || !meetingLink || !date || !time) {
        ...
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <div style="background-color: #f7f7f7; padding: 20px; border-radius: 8px; max-width: 600px; margin: auto;">
            ...
            <div style="background-color: #ffffff; padding: 20px; border-radius: 8px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);">
              ...
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Organizer:</strong> ${escapeHtml(name)}</p>
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Organizer's Email:</strong> <a href="mailto:${escapeHtml(email)}" style="color: #4a90e2;">${escapeHtml(email)}</a></p>
              <hr style="border: none; border-bottom: 1px solid #f0f0f0; margin: 15px 0;">
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Date:</strong> ${escapeHtml(date)}</p>
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Time:</strong> ${escapeHtml(time)}</p>
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Platform:</strong> ${escapeHtml(platform)}</p>
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Purpose:</strong> ${escapeHtml(description)}</p>
              <p style="margin: 10px 0;"><strong style="color: #4a90e2;">Meeting Link:</strong> <a href="${escapeHtml(meetingLink)}" style="color: #4a90e2; text-decoration: underline;">Join the Meeting</a></p>
            </div>
